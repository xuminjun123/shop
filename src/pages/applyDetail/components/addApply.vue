...
<template>
	<view>
		...
		<view class="w-11/12 mx-auto bg-white">
			<view class="p-2 form-item">
				<text> 姓名: </text>
				<input class="mt-1 uni-input" placeholder="请输入" focus v-model="formData.user_name" />
			</view>
			...
			<view class="p-2 form-item">
				...
				<view class="flex justify-between mt-1">
					...
					<tm-button :margin="[0]" size="small" :shadow="0" openType="getPhoneNumber" label="获取手机号"
						color="baseColor" @getphonenumber="getPhoneNumberCode"></tm-button>
				</view>
			</view>
		...
		<SelectDate :activeId="activeId" @sendSelectDate="sendSelectDate" @sendSelectTime="sendSelectTime"
			@sendSelectCount="sendSelectCount"></SelectDate>

		<view class="flex justify-between w-11/12 pb-5 mx-auto">
			<view class="flex-1 mr-2">
				<tm-button :shadow="0" text form-type="reset" label="取消" block color="baseColor" @click="resetApply">
				</tm-button>
			</view>
			<view class="flex-1">
				<!-- <tm-button form-type="submit" label="新增预约" block color="baseColor" @click="submit" :disabled="disabled">
				</tm-button> -->
				<tm-button form-type="submit" label="新增预约" block color="baseColor" @click="vote">
				</tm-button>
			</view>
		</view>
	</view>
	...
	<uni-popup ref="popupCoupon" background-color="#fff" type='bottom'>
		<view class="popup_content">
			<uni-section title="门票类型" type="line">
				<view class="example-body">
					<view class="tag_view">
						<uni-tag v-if="priceInfo.value.single !== '0' && priceInfo.value.single !==''" :disabled="selectCount==0"
							:inverted="selected==0?false:true" text="单人票" type="success" @click="favClick(0)"
							class="mar_right" />
						<uni-tag v-if="priceInfo.value.family !== '0' && priceInfo.value.family !==''" :disabled="selectCount<2"
							:inverted="selected==1?false:true" text="家庭票" type="success" @click="favClick(1)"
							class="mar_right" />
						<uni-tag v-if="priceInfo.value.group !== '0' && priceInfo.value.group !==''" :disabled="selectCount<5"
							:inverted="selected==2?false:true" text="团队票" type="success" @click="favClick(2)" />
					</view>
				</view>
			</uni-section>
			<uni-section title="请选择人数" type="line" v-if="selected ==2">
				<view class="tag_view">
					<template v-for="(item,index) of selectCount" :key="item">
						<uni-tag class="mar_right" :key="item" :inverted="peopleNum == item?false:true" v-if="item>=5"
							:text="item" type="success" @click="selectPeopleNum(item)" />
					</template>
				</view>
			</uni-section>
		...
		<tm-button :disabled="selectCount == 0" form-type="submit" label="新增预约" block color="baseColor"
			@click="payment">
		</tm-button>
	</uni-popup>
</template>
