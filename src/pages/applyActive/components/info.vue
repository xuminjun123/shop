  ...
  <view>
    ...
    <view v-if="showEdit">
      <view class="w-11/12 mx-auto bg-white">
        ...
        <view class="p-2 form-item">
          <view> 美术馆电话: </view>
          <view class="pt-2 text-sm text-gray-500"> 0512-68855558 </view>
        </view>
	     <view class="p-2 form-item">
          <view> 预约类型: </view>
          <view class="pt-2 text-sm text-gray-500"> {{}} </view>
        </view>
        <view class="p-2">
          <view> 预约时间: </view>
          <view class="pt-2 text-sm text-gray-500">
      ...
      <view class="flex items-center justify-center w-full mx-auto my-2">
        <tm-qrcode :option="qrcode" _class="mx-auto"></tm-qrcode>
      </view>
...
const qrcode = ref({ size: 650, str: props.info.qrcode });
const showEdit = ref(true); // 显示信息
const emptyDiv = ref(0);
const select = ref(1);
const selectDateNum = ref(0);
const selectId = ref(""); // 选中的id
const applytypeStr = ref("");
const selectDate = ref<number | null>(null); // 选中时间段有票的
const selectDateId = ref(""); // 选中的id

const applytime = computed(() => {
  const time = props.info.applytime;
  const applytime = format(Number(time), "yyyy年MM月dd日 HH时");
  return applytime;
});
// 确定时间
...
const selectTimeQuantum = (item: any, index: number) => {
  ...
  selectDateNum.value = item.countNum
};
