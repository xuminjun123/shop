<template>
  <view class="infoDetail-container">
    <view class="bg-white rounded-md">
      ...
      <view
        class="w-11/12 pb-1 pl-1 mx-auto text-xs c_o"
        style="color: #eb6120"
      >
        活动时间: {{ starttime }}至{{ endtime }}
      </view>
      <view
        class="prompt_box w-11/12 pb-1 pl-1 mx-auto text-xs"
        v-if="appointmentdata.typecode == 1"
      >
        <view
          class="flex_row"
          v-if="
            appointmentdata.pricedata.single !== '0' &&
            appointmentdata.pricedata.single
          "
        >
        ...
        <view
          class="flex_row"
          v-if="
            appointmentdata.pricedata.family !== '0' &&
            appointmentdata.pricedata.family
          "
        >
          ...
          <view class="text-xs"
            >{{
              appointmentdata.pricedata.family
            }}元（两成人一小孩或两成人两小孩）</view
          >
        </view>
        <view
          class="flex_row"
          v-if="
            appointmentdata.pricedata.group !== '0' &&
            appointmentdata.pricedata.group
          "
        >
          <view class="text-xs c_red">团体单人票价：</view>
          <view class="text-xs"
            >{{ appointmentdata.pricedata.group }}元*人数（2-10人）</view
          >
        </view>
      </view>
    </view>
...
function getDetail(id: string) {
  http
    ...
    .then((res: any) => {
      ...
      appointmentdata.typecode = res.appointmentdata.price.includes("免费")
        ? 0
        : 1;
